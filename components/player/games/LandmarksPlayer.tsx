    ...
    return (
      <div className="min-h-screen bg-night-900 flex flex-col items-center justify-center p-6">
        <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', bounce: 0.4 }} className="text-center">
          ...
          <h2 className="text-3xl font-black text-white mb-2">{state.phase === 'countdown' ? `Landmark ${state.currentIndex + 1}` : 'Leaderboard'}</h2>
          <div className="glass rounded-2xl px-6 py-4 mt-6 inline-block">
            <p className="text-white/40 text-xs mb-1">Your score</p>
            <p className="text-3xl font-black text-amber-400">{me.score.toLocaleString()} <span className="text-white/30 text-base">pts</span></p>
          </div>
        </motion.div>
      </div>
    )
    ...
    return (
      <div className="min-h-screen bg-night-900 flex items-center justify-center">
        <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="text-center">
          ...
          <h2 className="text-2xl font-black text-white">Landmarks done!</h2>
  ...
  return (
    <div className="min-h-screen bg-night-900 flex flex-col p-4 pt-6">
      <div className="flex items-center justify-between mb-4">
        <div>
          <p className="text-white/30 text-xs uppercase tracking-wider">Landmark {state.currentIndex + 1}/{state.questions.length}</p>
          <p className="text-amber-400/80 text-sm font-bold">{q.questionType === 'name' ? '🏷️ Name it' : '🌍 Which country?'}</p>
        </div>
      ...
      <AnimatePresence mode="wait">
        <motion.div key={`img-${state.currentIndex}`} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
          className="relative rounded-2xl overflow-hidden mb-4 flex-shrink-0" style={{ height: '45vw', maxHeight: '200px', minHeight: '130px' }}>
          {q.imageUrl ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={q.imageUrl} alt={q.name} className="w-full h-full object-cover" />
          ) : (
            <div className="w-full h-full bg-amber-900/30 flex items-center justify-center"><span className="text-5xl">🏛️</span></div>
          )}
          <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent" />
          {isRevealed && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute bottom-3 left-3 right-3">
              <p className="text-yellow-400 font-black text-base drop-shadow">🏛️ {q.name}</p>
              <p className="text-white/70 text-sm drop-shadow">📍 {q.city}, {q.country}</p>
            </motion.div>
          )}
        </motion.div>
      </AnimatePresence>
      ...
      <AnimatePresence>
        {isRevealed && (
          <motion.div initial={{ scale: 0.75, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.75, opacity: 0 }}
            ...
            {myAnswer?.correct ? '✅ Correct!' : myAnswer ? `❌ It's ${correctAns}` : "⏱️ Time's up!"}
          </motion.div>
        )}
      </AnimatePresence>
