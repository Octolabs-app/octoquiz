...
import type { RoomPublic, LandmarksState } from '@/lib/types'
import Timer from '@/components/ui/Timer'
import Scoreboard from '@/components/ui/Scoreboard'
import { useSound } from '@/hooks/useSound'

interface Props { room: RoomPublic; state: LandmarksState }

const OPTION_COLORS = ['#3b82f6', '#ef4444', '#22c55e', '#eab308']
const OPTION_LABELS = ['A', 'B', 'C', 'D']

export default function LandmarksHost({ room, state }: Props) {
  ...
  useEffect(() => { setImgError(false) }, [state.currentIndex])
        ...
        <div className="w-full max-w-2xl text-center">
          <motion.div initial={{ scale: 0.7, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', bounce: 0.4 }}>
            ...
            <h2 className="text-4xl font-black text-white mb-8">
              {state.phase === 'countdown' ? `Landmark ${state.currentIndex + 1} of ${state.questions.length}` : 'Leaderboard'}
            </h2>
          </motion.div>
    ...
    return (
      <div className="min-h-screen bg-night-900 flex items-center justify-center">
        <motion.div initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', bounce: 0.5 }} className="text-center">
          ...
          <h2 className="text-5xl font-black text-white">Landmarks Complete!</h2>
  ...
  const correctAns  = q.questionType === 'name' ? q.name : q.country

  return (
    <div className="min-h-screen bg-night-900 flex gap-0 no-select overflow-hidden">
      {/* ── Left: landmark image ── */}
      <div className="flex-1 relative overflow-hidden">
        <div className="absolute inset-x-0 bottom-0 h-48 bg-gradient-to-t from-night-900 to-transparent z-10 pointer-events-none" />

        <AnimatePresence mode="wait">
          <motion.div key={`img-${state.currentIndex}`}
            initial={{ opacity: 0, scale: 1.05 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }}
            transition={{ duration: 0.5 }} className="absolute inset-0">
            {q.imageUrl && !imgError ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={q.imageUrl} alt={q.name} className="w-full h-full object-cover"
                onError={() => setImgError(true)} />
            ) : (
              <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-amber-900/30 to-orange-900/30">
        ...
        <div className="absolute bottom-0 left-0 right-0 z-20 p-8">
            ...
            {revealed && (
              <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="mb-3">
                <div className="flex items-center gap-3 flex-wrap">
                  <span className="glass px-4 py-2 rounded-xl text-xl font-black text-yellow-400">
                    🏛️ {q.name}
                  </span>
                  <span className="glass px-4 py-2 rounded-xl text-lg font-bold text-white/70">
                    📍 {q.city}, {q.country}
                  </span>
                </div>
              </motion.div>
          ...
          <h2 className="text-3xl font-black text-white drop-shadow-lg">
            {revealed ? '✅ Revealed!' : q.questionText}
          </h2>
        </div>
      </div>
      ...
      <div className="w-[420px] flex-shrink-0 flex flex-col bg-night-900/95 border-l border-white/5 p-6 gap-4 overflow-y-auto">
        <div className="flex items-center justify-between">
          ...
          <span className="glass px-2 py-1 rounded-lg text-xs font-bold"
            style={{ color: q.questionType === 'name' ? '#f59e0b' : '#22c55e' }}>
            {q.questionType === 'name' ? '🏷️ Name it' : '🌍 Which country?'}
          </span>
          ...
          {q.options.map((opt, i) => {
            ...
            return (
              <motion.div key={`${state.currentIndex}-${i}`}
                ...
                animate={{ x: 0, opacity: revealed && !isCorrect ? 0.3 : 1, scale: revealed && isCorrect ? 1.03 : 1 }}
